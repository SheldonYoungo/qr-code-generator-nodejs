...
function deletePreviousQR() {
    const urlExtensions = ['.com', '.net', '.io', '.gob', '.org', '.us', '.co'];
    let currentQR = fs.readdirSync('public/qr_codes');

    let filesToDelete = [];

    urlExtensions.forEach(extension => {
        currentQR.forEach(qr => {
            if(qr.includes(extension)) {
               filesToDelete.push(qr);
            }
        })
    })

    filesToDelete.forEach(file => fs.unlinkSync(`public/qr_codes/${file}`))
}
